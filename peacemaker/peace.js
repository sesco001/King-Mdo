...
│ 🎭 clap        — Add claps to text
│ 🎭 slap/hug/kiss/punch — GIF reactions
┗▣
...
│ ✦ lyrics      — Song lyrics
│ ✦ toimg/sti   — Sticker to image
│ ✦ steal/take  — Clone a sticker
┗▣
...
//========================================================================================================================//
// ══════════════════════ WOLFBOT-STYLE NEW COMMANDS ══════════════════════════
//========================================================================================================================//
case 'rizz': case 'pickup': case 'flirt': {
    const rizzLines = [
...
case 'advice': {
    try {
        ...
        const tip = r?.slip?.advice || "Keep going, you're doing great!";
        reply(`💡 *ADVICE OF THE MOMENT*\n\n_"${tip}"_`);
    } catch (e) {
...
//========================================================================================================================//
case 'mock': case 'spongebob': {
    if (!text && !m.quoted?.text) return reply(`Usage: ${prefix}mock [text]`);
    const mockText = (text || m.quoted.text);
...
//========================================================================================================================//
case 'clap': {
    if (!text) return reply(`Usage: ${prefix}clap [text]`);
...
case 'love': case 'lovecalc': {
    const names = text || (m.quoted?.pushName || pushname);
    const pct = Math.floor(Math.random() * 101);
    const bar = '❤️'.repeat(Math.floor(pct / 10)) + '🖤'.repeat(10 - Math.floor(pct / 10));
    let emoji = pct >= 80 ? '💯' : pct >= 60 ? '😍' : pct >= 40 ? '😊' : pct >= 20 ? '😐' : '💔';
...
//========================================================================================================================//
case 'iq': {
    const name = text || pushname;
    const iqScore = Math.floor(Math.random() * 100) + 60;
    let level = iqScore >= 140 ? 'Genius 🧠' : iqScore >= 120 ? 'Very Smart 🤓' : iqScore >= 100 ? 'Average 😐' : iqScore >= 80 ? 'Below Average 😅' : 'Needs Help 💀';
    reply(`🧠 *IQ TEST*\n\n👤 *Name:* ${name}\n📊 *IQ Score:* ${iqScore}\n📝 *Level:* ${level}`);
    break;
}
...
//========================================================================================================================//
case 'rate': {
    if (!text) return reply(`Usage: ${prefix}rate [thing to rate]`);
...
//========================================================================================================================//
case 'roast': {
    const target = text || (m.quoted?.pushName || pushname);
    const roasts = [
        "You're so slow, you'd lose a race to a turtle with a broken leg.",
...
//========================================================================================================================//
case 'compliment': {
    const person = text || (m.quoted?.pushName || pushname);
    const compliments = [
...
//========================================================================================================================//
case 'toimg': case 'stickertoimg': case 'sti': {
    if (!m.quoted) return reply('Reply to a sticker with this command.');
    if (m.quoted.mtype !== 'stickerMessage') return reply('Only reply to a sticker.');
    try {
        await client.sendMessage(m.chat, { react: { text: '⏳', key: m.key } });
        const buf = await downloadMediaMessage(m.quoted.fakeObj || m.quoted, 'buffer', {});
        await client.sendMessage(m.chat, { image: buf, caption: '🖼️ Here is your sticker as an image!' }, { quoted: m });
        await client.sendMessage(m.chat, { react: { text: '✅', key: m.key } });
    } catch (e) {
        logError('TOIMG', e);
...
//========================================================================================================================//
case 'steal': case 'take': case 'clone': {
    if (!m.quoted) return reply('Reply to a sticker to steal it.');
    if (m.quoted.mtype !== 'stickerMessage') return reply('Only works on stickers!');
    try {
        await client.sendMessage(m.chat, { react: { text: '🔄', key: m.key } });
        const buf = await downloadMediaMessage(m.quoted.fakeObj || m.quoted, 'buffer', {});
        await client.sendMessage(m.chat, {
        ...
        logError('STEAL', e);
        reply('❌ Could not steal that sticker.');
    }
    break;
}
...
case 'slap': {
    try {
        const r = await fetchJson('https://nekos.life/api/v2/img/slap');
        await client.sendMessage(m.chat, { image: { url: r.url }, caption: `👋 *${pushname} slaps ${m.quoted?.pushName || 'someone'}!*` }, { quoted: m });
    } catch (e) { reply('❌ Could not fetch slap GIF.'); }
    break;
}
case 'hug': {
    try {
        const r = await fetchJson('https://nekos.life/api/v2/img/hug');
        await client.sendMessage(m.chat, { image: { url: r.url }, caption: `🤗 *${pushname} hugs ${m.quoted?.pushName || 'someone'}!*` }, { quoted: m });
    } catch (e) { reply('❌ Could not fetch hug GIF.'); }
    break;
}
case 'kiss': {
    try {
        const r = await fetchJson('https://nekos.life/api/v2/img/kiss');
        await client.sendMessage(m.chat, { image: { url: r.url }, caption: `😘 *${pushname} kisses ${m.quoted?.pushName || 'someone'}!*` }, { quoted: m });
    } catch (e) { reply('❌ Could not fetch kiss GIF.'); }
    break;
}
case 'punch': {
    try {
        const r = await fetchJson('https://nekos.life/api/v2/img/slap');
        await client.sendMessage(m.chat, { image: { url: r.url }, caption: `👊 *${pushname} punches ${m.quoted?.pushName || 'someone'}!*` }, { quoted: m });
    } catch (e) { reply('❌ Could not fetch punch GIF.'); }
    break;
}
...
case 'qr': case 'qrcode': {
    ...
    await client.sendMessage(m.chat, { image: { url: qrUrl }, caption: `📱 *QR Code*\n\n📝 *Content:* ${text}` }, { quoted: m });
    break;
}
...
case 'lyrics': case 'lyric': {
    ...
    try {
        await client.sendMessage(m.chat, { react: { text: '🎵', key: m.key } });
        const r = await fetchJson(`https://api.lyrics.ovh/v1/${encodeURIComponent(text.split(' ')[0])}/${encodeURIComponent(text.split(' ').slice(1).join(' ') || text)}`);
        if (!r?.lyrics) return reply('❌ Lyrics not found for that song.');
        const lyrics = r.lyrics.substring(0, 3500);
        reply(`🎵 *LYRICS*\n\n🎶 *${text.toUpperCase()}*\n\n${lyrics}${r.lyrics.length > 3500 ? '\n\n_...trimmed_' : ''}`);
    } catch (e) {
        reply('❌ Could not find lyrics. Try format: `.lyrics artist songname`');
    }
    break;
}
...
case 'translate': case 'tr': {
    if (!text) return reply(`Usage: ${prefix}translate [lang] [text]\nExample: ${prefix}translate en Habari yako`);
    const parts = text.split(' ');
    const lang = parts[0];
    const toTranslate = parts.slice(1).join(' ');
    if (!toTranslate) return reply(`Usage: ${prefix}translate [lang] [text]`);
    try {
        const r = await fetchJson(`https://api.mymemory.translated.net/get?q=${encodeURIComponent(toTranslate)}&langpair=auto|${lang}`);
